...
func validateStackSequences(pushed []int, popped []int) bool {
    ...
    i,o := 0,0
    for o < len(popped) {
        if i < len(pushed) && pushed[i] != popped[o] {
            stk = append(stk, pushed[i])
            i++
            continue
        }
        if i < len(pushed) {i++} // get rid of common element
        o++
        sidx := len(stk)-1
        for o < len(popped) && sidx >= 0 && stk[sidx] == popped[o] {
            sidx--
            o++
            //fmt.Println(sidx,o, stk, popped[o:])
        }
        stk = stk[:sidx+1]
    }
    if 0 == len(stk) {return true}
    return false
}
