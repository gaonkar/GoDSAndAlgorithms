...
func Add(b byte, i int) byte {
    r := int(b) - '0'
    r = (r + 10 + i) % 10 + int('0')
    return byte(r)
}
func Neighbors(s string, invalid map[string] bool) []string {
    r := make([]string, 0)
    b := []byte(s)
    if invalid[s] {return r}
    for i:=0; i < len(s); i++ {
        b[i] = Add(b[i], 1)
        str := string(b)
        if !invalid[str] {r = append(r, str)}
        b[i] = Add(b[i], -2)
        str = string(b)
        if !invalid[str] {r = append(r, str)}
        b[i] = Add(b[i], 1)
...
func openLock(deadends []string, target string) int {
    ...
    q := []string{"0000"}
    step :=0
    for _, inv := range(deadends) {invalid[inv] = true}
    // BFS
    for len(q) > 0 {
        fmt.Println(len(q))
        nm := make(map[string] bool) // map to remove duplicate nodes at the same level
        for _, n:= range(q) {
            if target == n {return step}
            visited[n]= true
            nb := Neighbors(n, invalid)
            for _, adj := range(nb) {
