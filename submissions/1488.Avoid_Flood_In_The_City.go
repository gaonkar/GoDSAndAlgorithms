...
// Solution 1 O(N^2) worst case due to dlist
/*
 Use sorted set in python. If we go by basic implement that using skiplist of binary tree. 
 */

func avoidFlood(rains []int) []int {
    ret := make([]int, len(rains))
    rained := make(map[int] int)
    dlist := make([]int, 0)
    for i, x := range(rains) {
        ret[i] = 1
        if x == 0 {
        ...
        if !ok { continue }
        i := 0
        for i < len(dlist) && (dlist[i] < idx) {i++}
        if len(dlist) <= i {return nil}
        ret[dlist[i]] = x
        dlist = append(dlist[:i], dlist[i+1:]...) //adding O(N)

        //fmt.Println(rained, x, idx, dlist, i, ret)
    }
    return ret
}
