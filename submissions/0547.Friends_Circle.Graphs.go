...
func findCircleNum(M [][]int) (ret int) {
    ...
    V := make([]bool, L)
    if L == 0 {return 0}
    var DFSCycle func(idx int)

    DFSCycle = func (idx int) {
        for j:=0; j < L; j++ { // only upper triangle
            if M[idx][j] == 1 && !V[j]{
                V[j] = true
                DFSCycle(j)
            }
    ...
    for i:=0; i < L;i++ {
        if V[i] {continue} // already visited
        V[i]=true
        DFSCycle(i)
        // note that even 2 friends make a circle here
        ret++
    }
